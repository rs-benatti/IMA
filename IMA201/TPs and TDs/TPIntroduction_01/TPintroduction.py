...
plt.plot(np.log(np.histogram(gradx(imbr),255)[0]))
plt.show()
...
imequal=histocum[np.uint8(im)]
print(imequal.shape)
(histo,bins)=np.histogram(imequal.reshape((-1,))*256,np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
viewimage(imequal)
...
'''
print(v.reshape((-1,)).size/256)
old = np.array([])
for i in range(0,256):
    new = np.append(old, np.ones(int(v.reshape((-1,)).size/256))*i)
    old = new
'''
...
(histo,bins)=np.histogram(unew.reshape((-1,)),np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
...
im=skio.imread('images/lena.tif')
grad = gradx(im)
plt.hist(grad.reshape((-1,)),bins=255)
plt.show()
viewimage(grad)
...
im_lowpass = filterlow(im)
viewimage(im_lowpass)
view_spectre(im_lowpass,option=2,hamming=True)
#%% FIN TP INTRODUCTION
