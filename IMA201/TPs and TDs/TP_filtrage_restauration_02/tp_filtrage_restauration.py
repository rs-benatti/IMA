...
def viewimage_task(im,foo, normalise=True,MINI=0.0, MAXI=255.0):
    t1 = Thread(target=viewimage, args=(im,))
...
kernel_gaus = get_gau_ker(2)
im = filtre_lineaire(im, kernel_gaus)
viewimage(im)
...
'''
 Appliquer un filtre lin´eaire `a une image puis utilisez la fonction filtre inverse 2
. Que constatezvous ? Que se passe-t-il si vous ajoutez tr`es peu de bruit `a l’image flout´ee avant de la restaurer
par la commade pr´ec´edente ?

'''
im=skio.imread('images/pyramide.tif')
kernel_gaus = get_gau_ker(2)
im = filtre_inverse(im, kernel_gaus)
viewimage(im)


def foo():
    im1 = im.copy()
    im1 = filtre_lineaire(im1, kernel_gaus)
    viewimage(im1)
...
def foo():
    im1 = im.copy()
    im1 = filtre_inverse(im1, kernel_gaus)
    viewimage(im1)
...
kernel_gaus = np.array([[0, 0, 1], [1, 1, 1], [0, 0, 1]])
im1 = filtre_lineaire(im, kernel_gaus)
viewimage(im1)
...
# On ouvre l'image et choise l'écart-type du bruit
im = skio.imread('images/carre_orig.tif')
inputDp = 5
im = noise(im, inputDp)

# Puis on calcule le rapport entre les écart-types avant et après filtrage
outputDpMedian = var_image(median_filter(im,typ=2,r=4), 0, 0, 50, 50)**0.5
dpRatioMedian = inputDp/outputDpMedian
print(outputDpMedian)

# Puis, on va parcourir les tailles du kernel et à chaque itération
# on va calculer le rapport entre les écart-types pour le filtrage par noyau 
# si on rencontre un valeur supérior a l'écart-type calculé avant
...
im = noise(im, inputDp)
'''
kernel_gaus = get_cst_ker(3)
im1 = filtre_lineaire(im, kernel_gaus)
im2 = median_filter(im,r=4)
viewimage(im1)
viewimage(im2)
...
inputDp = 5
im = noise(im, inputDp)
imWienerSpectre = wienerSpectre(im, np.ones((3, 3)))
viewimage(im)
viewimage(imWienerSpectre)
# %%
